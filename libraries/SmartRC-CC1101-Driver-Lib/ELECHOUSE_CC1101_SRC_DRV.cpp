...
void ELECHOUSE_CC1101::setSpi(void){
  ...
  #elif ESP32
  SCK_PIN = 12; MISO_PIN = 13; MOSI_PIN = 11; SS_PIN = 5;
  #else
  SCK_PIN = 12; MISO_PIN = 13; MOSI_PIN = 11; SS_PIN = 5;
  #endif
}
...
void ELECHOUSE_CC1101::setPAControl(int tx_en, int rx_en) {
  _tx_en_pin = tx_en;
  _rx_en_pin = rx_en;
  if (_tx_en_pin != -1) {
    pinMode(_tx_en_pin, OUTPUT);
    digitalWrite(_tx_en_pin, LOW);
  }
  if (_rx_en_pin != -1) {
    pinMode(_rx_en_pin, OUTPUT);
    digitalWrite(_rx_en_pin, LOW);
  }
}
